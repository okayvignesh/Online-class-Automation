...
ASSETS_PATH = OUTPUT_PATH / Path("./userreset_assets")
...
def confirmf():
    ...
    if o=="" or n=="":
        ...
        try:
            cursor.execute("select * from (select uname from user_reset order by reset_date desc) where rownum=1")
            s=cursor.fetchone()
            s1=str(s[0])
            sql="UPDATE user_tb SET pwd='{}' where uname='{}'".format(o,s1)
            cursor.execute(sql)
            con.commit()

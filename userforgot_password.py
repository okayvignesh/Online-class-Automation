...
ASSETS_PATH = OUTPUT_PATH / Path("./userforgot_assets")
...
def sendf():
    ...
    if mailid=="":
        ...
        cursor.execute("select uname from user_tb where email='{}'".format(mailid))
        data=[]
        res=cursor.fetchone()
        if res==None:
            ...
            messagebox.showinfo("SUCCESS",'OTP has been sent to your mail!')
...
def verify():
    ...
    if o==otp:
        ...
        sql="INSERT into user_reset(uname, reset_date) VALUES ('{}', sysdate)".format(u)
        try:
            cursor.execute(sql)
            con.commit()
